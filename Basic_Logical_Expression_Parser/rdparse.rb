...
class Log_expr
  ...
  attr_reader :logParser
  
  def initialize
    @logParser = Parser.new("Log Parser") do
      @var_table = Hash.new()
      token(/\s+/)
      token(/\w+/) {|m| m }
      token(/./) {|m| m }
      ...
      rule :ASSIGN do
        match('(', 'set', :VAR, :EXPR,')'){|_, _, a, b, _| @var_table[a] = b }
      end
      ...
      rule :VAR do
        #Ternary operator (if true: result : else)
        match(/[a-zA-Z]+/) {|asd|
          @var_table.key?(asd) ? @var_table[asd] : asd}
      end
    end
...
# [logParser] (not false)
# => true
# [logParser] (set a true)
# => true
# [logParser] (set b false)
# => false
# [logParser] (or a b)
# => true
# [logParser] (and a b)
