  ...
  def new_value(connector)
    ...
    if [out].include?(connector)
      if a.has_value? and not b.has_value?
        # val = out - a
        # Samma som rad 171 - 173(typ)
        val = out.value.send(inverse_op, a.value)
        b.assign(val, self)
      end
      if b.has_value? and not a.has_value?
        # val = out - b
...
def celsius2fahrenheit
  f = Connector.new("f")
  c = Connector.new("c")
  # F till C -> Substrahera 32, Multiplicera med 5, Dela med 9
  # Tc = (Tf - 32) / 1.8 -------//(1.8 är 9/5)
  # C till F -> Multiplicera med 9, Dela med 5, addera 32
  # Tf = Tc * 1.8 + 32
  # Så (C * 1.8 = F - 32 = fConnector)
  c1 = ConstantConnector.new("c1", 1.8)
  c32 = ConstantConnector.new("c32", 32)

  fConnector = Connector.new("9c")

  Multiplier.new(c, c1, fConnector)
  Adder.new(fConnector, c32, f)
