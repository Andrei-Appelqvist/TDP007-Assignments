...
class ConstraintParser < Parser
    ...
    @parser=super("constraints") do
      ...
      rule :expr do
        ...
        match(:expr, '-', :term) do |a, _, b|
          conn_a,conn_b,conn_c=get_connectors(a,'-',b)
          # a-b=c <=> a=b+c
          Adder.new(conn_a,conn_b,conn_c).change_operator
        end
      ...
      rule :term do
        ...
        match(:term, '/', :atom) do |a, _, b|
          conn_a,conn_b,conn_c=get_connectors(a,'/',b)
          # a/b=c <=> b*c=a
          Multiplier.new(conn_a,conn_b,conn_c).change_operator
        end
  ...
  def get_connectors(conn_a,op,conn_b)
    #ny kod
    #Hämtar connector så att man kan hämta namn
  ...
  def replace_conn(lh,rh)
    ...
    elsif lh.is_a?(ArithmeticConstraint)
           exp,conn=lh,rh_conn
